...
import { ArrowRight, Star, ShoppingCart, X, Check, Minus, Plus, ChevronLeft, ChevronRight, Coffee, Search, AlertCircle } from 'lucide-react';
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(full as any).flavors.map((f: { name: string; image?: string }) => (
                  ...
                  >
                    <div className="h-9 flex items-center justify-center relative overflow-hidden" style={{ backgroundColor: f.image ? undefined : '#f4efe4' }}>
                      {f.image ? (
                ...
                {((full as any).addons as { name: string; price: number }[]).map((a) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ backgroundColor: sel ? '#b88917' : 'transparent', color: sel ? '#fff' : '#2e210e', borderColor: sel ? '#b88917' : 'rgba(46,33,14,0.2)', fontFamily: 'var(--font-body)' }}
                    >
                      {a.name}
...
function TrackOrderWidget() {
  ...
  const [result, setResult] = useState<import('../context/AppContext').OnlineOrder | null>(null);
  ...
  const STATUS_COLOR: Record<string, string> = {
    'Payment Pending': '#b88917',
    'Payment Confirmed': '#007AFF',
    'Preparing': '#b88917',
    'Delivering / Ready for Pickup': '#556419',
    'Delivered': '#556419',
  };
  ...
  const doSearch = useCallback(async (q: string, silent = false) => {
    ...
    const qPhone = q.trim().replace(/\s/g, '');
    ...
    const found = customerOrders.find(
      o => o.id.toUpperCase() === qUpper || o.phone.replace(/\s/g, '') === qPhone
    );
    setSearched(true);
  ...
  // Poll every 10 seconds when result is active and not yet delivered
  useEffect(() => {
    if (result && result.status !== 'Delivered') {
      pollRef.current = setInterval(() => doSearch(query, true), 10_000);
    }
  ...
  return (
    <div
      ...
      style={{ backgroundColor: '#fff', border: '1px solid rgba(61,32,16,0.1)' }}
    >
      <div className="flex gap-3">
        <input
          ...
          placeholder="Order code (TSC-XXXXX) or phone number"
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2.5 rounded-xl font-semibold text-sm transition-all hover:opacity-90 flex items-center gap-1.5"
      ...
      {searched && notFound && (
        <div className="mt-3 flex items-start gap-2 p-3 rounded-xl" style={{ backgroundColor: '#fff5f5', border: '1px solid #fed7d7' }}>
          <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#e53e3e' }} />
          <p className="text-xs" style={{ color: '#e53e3e' }}>
            No order found for <strong>"{query}"</strong>. Check your order code or phone number.
          </p>
        </div>
      )}

      {result && (
        <div className="mt-3 p-3.5 rounded-xl" style={{ backgroundColor: '#FAF5EE', border: '1px solid rgba(61,32,16,0.1)' }}>
          <div className="flex items-center justify-between mb-2">
            <div>
              <p className="text-xs uppercase tracking-widest font-semibold" style={{ color: '#9C7A5A' }}>Order</p>
              <p className="font-bold text-base tracking-wider" style={{ color: '#3D2010', fontFamily: 'var(--font-display)' }}>
                {result.id}
              </p>
            </div>
            <span
              ...
              style={{
                backgroundColor: `${STATUS_COLOR[result.status] || '#b88917'}18`,
                color: STATUS_COLOR[result.status] || '#b88917',
              }}
            >
              {result.status}
            </span>
      ...
      {!searched && (
        <p className="text-xs mt-3 text-center" style={{ color: '#9C7A5A', fontFamily: 'var(--font-body)' }}>
          Your order code was shown on the confirmation page.
        </p>
      )}
    </div>
  );
...
function ReviewsSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback: show optimistically if backend fails
      const newReview: Review = {
        id: `r-${Date.now()}`,
        name: name.trim(),
        rating,
    ...
    <section className="py-14 sm:py-20 px-6" style={{ backgroundColor: '#FFFFFF' }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-4 mb-8">
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="w-10 h-10 rounded-full flex items-center justify-center transition-all disabled:opacity-30" style={{ backgroundColor: page === 0 ? 'rgba(46,33,14,0.08)' : '#b88917', color: page === 0 ? '#2e210e' : '#fff' }}>
            <ChevronLeft className="w-5 h-5" />
          </button>
          <div className="flex gap-1.5">
            {Array.from({ length: totalPages }).map((_, i) => (
              <button key={i} onClick={() => setPage(i)} className="rounded-full transition-all" style={{ width: i === page ? '24px' : '8px', height: '8px', backgroundColor: i === page ? '#b88917' : 'rgba(46,33,14,0.2)' }} />
            ))}
          </div>
          <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="w-10 h-10 rounded-full flex items-center justify-center transition-all disabled:opacity-30" style={{ backgroundColor: page >= totalPages - 1 ? 'rgba(46,33,14,0.08)' : '#b88917', color: page >= totalPages - 1 ? '#2e210e' : '#fff' }}>
        ...
        <div className="text-center">
          ...
          {!showForm ? (
            <button onClick={() => setShowForm(true)} className="px-6 py-3 rounded-full font-semibold text-sm transition-all hover:opacity-90" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>
              Write a Review
            </button>
          ) : (
            <form onSubmit={handleSubmit} className="max-w-md mx-auto rounded-2xl p-5 text-left" style={{ backgroundColor: '#fff', border: '1px solid rgba(46,33,14,0.1)', boxShadow: '0 4px 20px rgba(0,0,0,0.07)' }}>
              ...
              <div className="mb-3">
                ...
                <input type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="Your name" className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none" style={{ borderColor: 'rgba(46,33,14,0.2)', backgroundColor: '#FAFAFA', color: '#2e210e', fontFamily: 'var(--font-body)' }} />
              </div>
              <div className="mb-3">
              ...
              <div className="mb-4">
                ...
                <textarea value={text} onChange={e => setText(e.target.value)} required placeholder="Tell us about your experience..." rows={3} className="w-full px-4 py-2.5 rounded-xl border text-sm outline-none resize-none" style={{ borderColor: 'rgba(46,33,14,0.2)', backgroundColor: '#FAFAFA', color: '#2e210e', fontFamily: 'var(--font-body)' }} />
              </div>
              <div className="flex gap-2">
                <button type="submit" className="flex-1 py-2.5 rounded-xl font-semibold text-sm transition-all hover:opacity-90" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>Submit Review</button>
                <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2.5 rounded-xl text-sm transition-all hover:opacity-80" style={{ backgroundColor: 'rgba(46,33,14,0.08)', color: '#2e210e', fontFamily: 'var(--font-body)' }}>Cancel</button>
              </div>
            </form>
          )}
        </div>
...
export default function Home() {
  ...
  const handleAdd = (item: typeof featured[0], opts: { size: string; flavor: string; addons: string[]; qty: number }) => {
    ...
    if (opts.size === 'L' || opts.size === '11in') price = (full as any).priceL || price;
    const addonTotal = ((full as any).addons || [])
      .filter((a: any) => opts.addons.includes(a.name))
      .reduce((s: number, a: any) => s + (a.price || 0), 0);
    const stableId = `${item.id}__${opts.size}__${opts.flavor}__${opts.addons.sort().join(',')}`;
    addToCart({
      id: stableId,
      name: item.name,
    ...
    <div style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 overflow-hidden">
          <div style={{ position: 'absolute', inset: '-15%', transform: `translateY(${scrollY * 0.38}px)`, willChange: 'transform' }}>
            <ImageWithFallback src={heroImg} alt="Teascape Bugo Cafe" className="w-full h-full object-cover" style={{ objectPosition: 'center 10%' }} />
          </div>
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(20,12,4,0.55) 0%, rgba(20,12,4,0.42) 45%, rgba(20,12,4,0.78) 100%)' }} />
        </div>

        <div className="relative z-10 px-6 max-w-4xl mx-auto w-full">
          <motion.h1 initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, ease: 'easeOut' }} className="text-white mb-4" style={{ fontFamily: 'var(--font-display)', fontSize: '64px', fontWeight: 700, fontStyle: 'italic', lineHeight: 1.1, textShadow: '0 2px 16px rgba(0,0,0,0.55)' }}>
            Escape the<br />Reality with us
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75, delay: 0.15, ease: 'easeOut' }} className="text-white/85 mb-8 leading-relaxed" style={{ fontSize: 'clamp(0.85rem, 1.8vw, 1rem)', fontFamily: 'var(--font-body)', maxWidth: '560px', textShadow: '0 1px 8px rgba(0,0,0,0.45)' }}>
            Your cozy escape in the heart of Bugo CDO.<br />
            Good tea, great food, and a warm atmosphere to call your own.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.28, ease: 'easeOut' }} className="flex flex-wrap gap-3">
            <Link to="/menu" className="px-6 py-3 rounded-md font-semibold text-sm transition-all hover:opacity-90 hover:scale-105" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>ORDER NOW</Link>
            <Link to="/menu" className="px-6 py-3 rounded-md font-semibold text-sm transition-all hover:scale-105" style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.35)', color: '#fff', fontFamily: 'var(--font-body)' }}>VIEW MENU</Link>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <FadeIn delay={0.05} className="flex items-center justify-center order-2 md:order-1">
              <div style={{ transform: 'rotate(-3deg)', backgroundColor: '#fff', padding: '10px 10px 28px 10px', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', maxWidth: '380px', width: '100%' }}>
                <div className="overflow-hidden h-64 md:h-80">
                  <ImageWithFallback src={storyImg} alt="Teascape Bugo interior" className="w-full h-full object-cover" />
                </div>
              </div>
            </FadeIn>
            <FadeIn delay={0.15} className="order-1 md:order-2">
              <p className="text-xs uppercase tracking-[0.22em] mb-3" style={{ color: '#b88917', fontFamily: 'var(--font-body)', fontWeight: 600 }}>Our Story</p>
              <h2 style={{ fontFamily: 'var(--font-display)', color: '#556419', fontSize: 'clamp(1.7rem, 3vw, 2.2rem)', fontWeight: 700, fontStyle: 'italic', lineHeight: 1.25 }} className="mb-4">More than just a milk tea</h2>
              <p className="leading-relaxed mb-4 text-sm" style={{ color: '#5C3A1A', fontFamily: 'var(--font-body)' }}>Teascape Bugo was born from a love of cozy spaces and great flavors, bringing the best of Thai-inspired milk tea culture to the heart of Bugo, CDO. We believe every visit should feel like a warm escape from the everyday.</p>
              <p className="leading-relaxed mb-6 text-sm" style={{ color: '#5C3A1A', fontFamily: 'var(--font-body)' }}>From our signature Cheesecake Milk Tea to hearty Korean Ramen and crispy pizzas, we've crafted a menu that has something for everyone — because food tastes better when shared.</p>
              <Link to="/about" className="inline-flex items-center gap-2 font-semibold text-sm" style={{ color: '#b88917', fontFamily: 'var(--font-body)' }}>Read our full story <ArrowRight className="w-4 h-4" /></Link>
            </FadeIn>
          </div>
        </div>
      </section>
      ...
      <section className="py-14 sm:py-20 px-6" style={{ backgroundColor: '#f4efe4' }}>
        <div className="max-w-6xl mx-auto">
          <FadeIn className="text-center mb-10">
            <p className="text-xs uppercase tracking-[0.22em] mb-3" style={{ color: '#b88917', fontFamily: 'var(--font-body)', fontWeight: 600 }}>Favorites</p>
            <h2 style={{ fontFamily: 'var(--font-display)', color: '#556419', fontSize: 'clamp(1.8rem, 3.5vw, 2.4rem)', fontWeight: 700, fontStyle: 'italic' }}>Must-Try Items</h2>
          </FadeIn>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {featured.map((item, i) => (
              <FadeIn key={item.id} delay={i * 0.07}>
                <div className="rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all group cursor-pointer h-full" style={{ backgroundColor: '#fff', border: '1px solid rgba(46,33,14,0.08)' }} onClick={() => setCustomizeItem(item)}>
                  <div className="relative h-40 sm:h-48 overflow-hidden">
                    {item.image ? (
                      <ImageWithFallback src={item.image} alt={item.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: '#f4efe4' }}><ShoppingCart className="w-10 h-10 text-gray-300" /></div>
                    )}
                    {item.isBestseller && (
                      <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>
                  ...
                  <div className="p-4 flex flex-col">
                    ...
                    <h3 className="font-semibold mb-1 leading-snug text-sm" style={{ color: '#556419', fontFamily: 'var(--font-display)' }}>{item.name}</h3>
                    <p className="text-xs mb-3 line-clamp-2" style={{ color: '#7A5030', fontFamily: 'var(--font-body)' }}>{item.description}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <span className="font-semibold text-sm" style={{ color: '#2e210e', fontFamily: 'var(--font-body)' }}>P{item.priceM || item.price}{item.priceL && <span className="text-xs text-gray-400"> /M</span>}</span>
                      <div onClick={e => { e.stopPropagation(); setCustomizeItem(item); }} className="w-11 h-11 rounded-full flex items-center justify-center cursor-pointer" style={{ backgroundColor: '#556419', color: '#fff' }}>
          ...
          <div className="text-center mt-8">
            <Link to="/menu" className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm transition-all hover:opacity-90" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>
      ...
      {/* ── Track Your Order ──────────────────────────────────────────────── */}
      <section className="py-12 px-6" style={{ backgroundColor: '#FAF5EE' }}>
        <div className="max-w-lg mx-auto">
          <FadeIn className="text-center mb-6">
            <p className="text-xs uppercase tracking-[0.22em] mb-3" style={{ color: '#b88917', fontFamily: 'var(--font-body)', fontWeight: 600 }}>Order Status</p>
            <h2 style={{ fontFamily: 'var(--font-display)', color: '#556419', fontSize: 'clamp(1.6rem, 3vw, 2rem)', fontWeight: 700, fontStyle: 'italic' }}>Track Your Order</h2>
            <p className="text-sm mt-2" style={{ color: '#5C3A1A', fontFamily: 'var(--font-body)' }}>Enter your order code or phone number to check your status.</p>
          </FadeIn>
          <FadeIn delay={0.1}><TrackOrderWidget /></FadeIn>
        </div>
      </section>
      ...
      <section className="py-14 sm:py-20 px-6">
        <FadeIn>
          <div className="max-w-4xl mx-auto rounded-3xl text-center py-14 px-6" style={{ background: 'linear-gradient(135deg, #1a0e05 0%, #2e210e 55%, #4a3010 100%)' }}>
            <p className="text-xs uppercase tracking-[0.22em] mb-3" style={{ color: '#b88917', fontFamily: 'var(--font-body)', fontWeight: 600 }}>Ready to Order?</p>
            <h2 className="text-white mb-4" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 700 }}>Order Online, Pick Up or Delivery</h2>
            <p className="text-white/70 mb-7 max-w-md mx-auto text-sm leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>Skip the wait. Order your favorites online and enjoy fast delivery or pickup from Teascape.</p>
            <Link to="/menu" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-semibold transition-all hover:scale-105 text-sm" style={{ backgroundColor: '#b88917', color: '#fff', fontFamily: 'var(--font-body)' }}>
      ...
      {customizeItem && <CustomizeModal item={customizeItem} onClose={() => setCustomizeItem(null)} onAdd={opts => handleAdd(customizeItem, opts)} />}

      {toast && (
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-5 py-3 rounded-full shadow-xl" style={{ backgroundColor: '#556419', color: '#fff', fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600 }}>
